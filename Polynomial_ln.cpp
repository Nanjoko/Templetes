...
int n, r[maxn], bit[maxn]; u64 f_1[maxn], f_2[maxn], inv[maxn], fac[maxn], tmp[maxn];
...
inline void Polynomial_inverse(u64 *a, u64 *b, u64 limit) {
...
inline void Derivate(u64 *a, u64 *b, int len) {
  for(int i = 1; i < len; ++i) b[i - 1] = i * a[i] % mod; b[len - 1] = 0;
}

inline void Integral(u64 *a, u64 *b, int len) {
  for(int i = 1; i < len; ++i) b[i] = a[i - 1] * Fast_pow(i, mod - 2) % mod; b[0] = 0;
}

inline void Ln(u64 *a, u64 *b, int len) {
  Derivate(a, fac, len), Polynomial_inverse(a, inv, len);
  int limit = len << 1;
  Number_theory_transform(fac, limit, 1);
  Number_theory_transform(inv, limit, 1);
  for(int i = 0; i < limit; ++i) fac[i] = fac[i] * inv[i] % mod;
  Number_theory_transform(fac, limit, -1);
  Integral(fac, b, len);
}
...
int main(int argc, char const *argv[])
{
  ...
  Ln(f_1, f_2, Limit(n - 1));
  for(int i = 0; i < n; ++i) printf("%d ", f_2[i]);
